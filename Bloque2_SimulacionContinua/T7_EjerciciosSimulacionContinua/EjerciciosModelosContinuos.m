...
grid on
hold off
%% Ejercicio 2: Ecuación Van der Pol
% Limpieza del entorno de trabajo
...
% parámetros de la configuración a continuación:
% Planteamiento
% $${x^{\prime } }^{\prime } =-\mu \left(x^2 -1\right){\;x}^{\prime } -x$$
% Configuración usuario
% Constantes del modelo
...
signal_x = signals{1}.Values;
signal_dx = signals{2}.Values;
x = signal_x.Data(:, 1);
dx = signal_dx.Data(:, 1);

plot(signal_x.Data(:, 2), signal_dx.Data(:, 2), pl.linespec, ...
    MarkerSize=pl.mkr.sz, ...
    DisplayName="(x, dx)");
xlabel('x')
ylabel('dx')
title('Ec Van der Pol (\mu = 0)')
grid on
axis square
hold off

plot(signal_x.Data(:, 1), signal_dx.Data(:, 1), pl.linespec, ...
    MarkerSize=pl.mkr.sz, ...
    DisplayName="(x, dx)");
xlabel('x')
ylabel('dx')
title('Ec Van der Pol (\mu = 5)')
grid on
axis square
hold off

plot(signal_x.Time, signal_dx.Data(:, 1), pl.linespec, ...
    MarkerSize=pl.mkr.sz, ...
    DisplayName="(x, dx)");
xlabel('x')
ylabel('dx')
...
axis square
hold off
%% Ejercicio 4: Ecuaciones del Lotka-Volterra
% Limpieza del entorno de trabajo
...
% parámetros de la configuración a continuación:
% Planteamiento
% $$\dot{x} =\left(a-b\;y\right)\;x$$
% 
% $$\dot{y} =\left(d\;x-c\right)\;y$$
% 
% $$\begin{array}{l}a=2\ldotp 7\\b=0\ldotp 7\\c=1\\d=3\end{array}$$
% Configuración usuario
% Constantes del modelo
...
model_name = "Ejer4_EcLotkaVolterra";
...
grid on
hold off
% min_val, i_min = min(signal.Data)
% Para saber cuando vuelve al máximo
[m, i] = max(signal.Data(6:end));

fprintf("En t=%.2fs se alcanza de nuevo el valor máximo %f\n", signal.Time(i+5), m)

% signal.Data(abs(signal.Data) <= 0.1)
%% Ejercicio 10: Respuesta escalón unitario
% Limpieza del entorno de trabajo
...
model_name = "Ejer10_RespuestaEscalonUnitario";
...
% title settings
labels.title.name = "Respuesta Escalón unitario";
labels.title.interpreter = "none";
% Ejecución simulación (Automática)
% Al final de este livescript se encuentra la definición de la función sim_config, 
% función que simula el modelo de nombre model_name para una configuración de 
...
% index = find(abs(signal.Data(1:end-1) - signal.Data(2:end)) < 0.1, 1);
% fprintf("Se estabiliza en t=%.2f\n", signal.Time(index))
        plot(signals{1}.Values.Data, signals{2}.Values.Data, pl.linespec, ...
            MarkerSize=pl.mkr.sz, ...
            DisplayName=signal.Name);
